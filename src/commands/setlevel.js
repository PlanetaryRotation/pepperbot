...
export default {
  ...
  execute(message, args) {
    ...
    if (userPermissionPosition <= targetPermissionPosition) {
      action.reply(
        message,
        "you cannot change the level of someone with a higher or equal role to you"
      );
      return;
    } else {
      if (
        ...
        levels.setLevel(message.guild.id, args[0], args[1]);
        action.reply(message, `set level of ${args[0]} to ${args[1]}`);
      } else {
        action.reply(
          message,
